...
if __name__ == '__main__':
    ...
    for tweet in json_data:
        if 'text' in tweet.keys():
            ...
            if vs['neg'] >= NEG_THRESHOLD and vs['compound'] < 0:
                print(tweet_count, '-', vs)
                all_info = {'tweet': text}
                all_info = {**all_info, **vs}
                all_text_score.append(all_info)
    df = pd.DataFrame(all_text_score)
    df.reset_index(inplace=True)
    df.set_index('tweet', inplace=True)
    df.sort_values(by=['neg'], ascending=False, inplace=True)
    print(df.shape)
    print(df.head())
    print('Total Tweet processed - ', tweet_count)
    print('Writing in CSV file')
    df.to_csv('all_tweet_scores.csv')
